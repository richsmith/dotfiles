...
;; Refresh package archives on demand: if an install fails (typically because
;; the cached archive contents are stale), refresh once and retry.
(define-advice package-install (:around (orig pkg &rest args) refresh-on-miss)
  (condition-case err
      (apply orig pkg args)
    (error
